...
public class rolldeviation90 : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        float z_recalc90 = Mathf.Repeat((float)Math.Round(target.localRotation.eulerAngles.y, 2), 360) / 2 - 180 * math.floor(Mathf.Repeat((float)Math.Round(target.localRotation.eulerAngles.y, 2), 360) / 180);
